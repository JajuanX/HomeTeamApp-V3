...
const useEditBusinessForm = () => {
	...
	const handle_submit = (user, businessId) => {
		...
		const userRef = (id) => firestore.doc(`users/${id}`)
		...
		updateBusinessRef.update({
			...
			.then(() => {
				mutate(`/api/business/${businessId}`)
					.then(router.push(`/business/${businessId}`))
			})
			.catch((error)=> {
				console.error(error);
			})
	...
	const handle_upload = (photo, fileName, targetName, imageDimensions, businessId, user) => {
		const updateBusinessRef = firestore.collection('businesses').doc(businessId);
		const updateBusinessDetailsRef = firestore.collection('businesses').doc(businessId).collection('additional').doc('details');
		const userRef = (id) => firestore.doc(`users/${id}`)
	...
	const handle_uploadChange = async (event, id, _user) => {
		...
		if (event.target.files[0]) {
			...
			try {
				Resizer.imageFileResizer(
					...
					0,
					async (uri) => {
						handle_upload(uri, fileName, targetName, imageDimensions, id, _user)
					},
					'blob',
					200,
					200,
